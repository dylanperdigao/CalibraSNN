...
class Net_FFSNN_1h(nn.Module, BaseNet_FFSNN):
    def __init__(self, topology, params, input=None):
        ...
        self.lif2 = snn.Leaky(beta=self.betas[1], threshold=self.thresholds[1], spike_grad=self.spike_grad, learn_beta=True, learn_threshold=True, output=True)
        # Architecture
        self.architecture = f"""
        =====================
    ...
    def forward(self, x):
        ...
        for _ in range(self.num_steps):
            ...
            # Layer2/3
            cur2 = self.fc2(spk1.flatten(1))
            spk2, mem2 = self.lif2(cur2, mem2)
            # Record the final layer
            cur_last_rec.append(cur2)
            spk_last_rec.append(spk2)
            mem_last_rec.append(mem2)
